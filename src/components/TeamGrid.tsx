...
function SocialButton({ href, icon, label }: { href: string; icon: React.ReactNode; label: string }) {
  return (
    <motion.a
      ...
      className="w-8 h-8 rounded-lg bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 flex items-center justify-center text-slate-600 dark:text-[#94A3B8] hover:text-[#2563EB] dark:hover:text-[#60A5FA] hover:border-[#60A5FA] transition-all duration-200"
    >
      {icon}
    </motion.a>
  );
  ...
  return (
    ...
    >
      ...
      <p className="text-slate-500 dark:text-[#64748B] text-sm max-w-md mx-auto">
        ...
        <a href="/studio" className="text-[#2563EB] dark:text-[#60A5FA] underline font-medium">Content CMS</a>.
      </p>
    </motion.div>
  );
}
  ...
  return (
    <motion.div
      ...
      className={`group relative rounded-2xl border bg-white dark:bg-white/[.04] p-6 flex flex-col gap-4 shadow-xl shadow-slate-900/5 dark:shadow-none transition-all duration-300 ${
        member.isFeatured
          ? "border-[#60A5FA] bg-amber-500/5 dark:bg-[#60A5FA]/[.03]"
          : "border-slate-200 dark:border-white/[.07] hover:border-[#60A5FA]/50"
      }`}
    >
      {/* Featured badge */}
      {member.isFeatured && (
        <div className="absolute top-4 right-4">
          <span className="text-[10px] hf-mono px-2 py-0.5 rounded-full bg-[#60A5FA]/20 text-[#2563EB] dark:text-[#60A5FA] border border-[#60A5FA]/40 tracking-widest font-semibold uppercase">
            Featured
          </span>
        </div>
      )}
      ...
      <div className="flex items-center gap-4">
        <div className="relative flex-shrink-0">
          <div className="w-16 h-16 rounded-xl overflow-hidden border-2 border-slate-200 dark:border-white/10 group-hover:border-[#60A5FA] transition-colors duration-300">
            {photoUrl ? (
              <Image
                src={photoUrl}
              ...
              <div className="w-full h-full bg-gradient-to-br from-[#60A5FA] to-[#1D4ED8] flex items-center justify-center text-white font-bold text-lg hf-display">
                {initials}
              </div>
            )}
          </div>
          {/* Active dot */}
          <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full bg-white dark:bg-[#070B14] flex items-center justify-center">
            <div className="w-2 h-2 rounded-full bg-emerald-500" />
          </div>
        </div>

        <div className="min-w-0">
          <h3 className="font-bold text-slate-900 dark:text-white text-base leading-tight group-hover:text-[#2563EB] dark:group-hover:text-[#60A5FA] transition-colors truncate">
            {member.name}
          </h3>
          <p className="text-[#2563EB] dark:text-[#60A5FA] text-xs font-semibold mt-0.5">{member.role}</p>
      ...
      <div className="flex flex-col gap-1.5">
        {member.researchArea && (
          <div className="flex items-center gap-2 text-[11px] text-slate-600 dark:text-[#94A3B8]">
            <BookOpen size={13} className="text-[#2563EB] dark:text-[#60A5FA] flex-shrink-0" />
            <span className="truncate">{member.researchArea}</span>
          </div>
        )}
        {member.institution && (
          <div className="flex items-center gap-2 text-[11px] text-slate-600 dark:text-[#94A3B8]">
            <GraduationCap size={13} className="text-[#2563EB] dark:text-[#60A5FA] flex-shrink-0" />
      ...
      <div className="max-w-6xl mx-auto px-6 relative z-10">
        ...
        >
          <div className="flex items-center justify-center gap-3 mb-4">
            <div className="h-px w-10 bg-gradient-to-r from-transparent to-[#2563EB] dark:to-[#60A5FA]" />
            <span className="text-[#2563EB] dark:text-[#60A5FA] text-[11px] hf-mono font-semibold tracking-[.18em] uppercase">
              Research Group
            </span>
            <div className="h-px w-10 bg-gradient-to-l from-transparent to-[#2563EB] dark:to-[#60A5FA]" />
          </div>
          <h1 className="hf-display text-4xl md:text-5xl font-extrabold text-slate-900 dark:text-white tracking-tight">
            Meet the <span className="text-[#2563EB] dark:text-[#60A5FA]">Team</span>
          </h1>
        ...
        {members.length === 0 ? (
          ...
          <>
            ...
            {featured.length > 0 && (
              <div className="mb-8">
                <p className="text-[11px] hf-mono text-[#2563EB] dark:text-[#60A5FA] font-bold tracking-widest uppercase mb-4">
        ...
        >
          <a
            href="/#contact"
            className="inline-flex items-center gap-2 px-8 py-3 rounded-xl border border-[#60A5FA] text-slate-900 dark:text-[#60A5FA] bg-white dark:bg-transparent hf-display font-bold text-[14px] tracking-wide hover:bg-[#60A5FA] hover:text-white dark:hover:bg-[#60A5FA] dark:hover:text-slate-950 transition-all duration-200 shadow-md"
          >
            <Mail size={15} />
