  ...
  return (
    <motion.button
      ...
      className={`relative w-9 h-9 rounded-xl flex items-center justify-center transition-all duration-300 border ${
        isDark
          ? "bg-white/5 text-[#60A5FA] border-white/10 hover:bg-white/10 hover:border-[#60A5FA]/40 shadow-inner"
    ...
    >
      ...
      >
        {isDark ? <Sun size={18} className="text-[#60A5FA]" /> : <Moon size={18} className="text-amber-600" />}
      </motion.div>
    </motion.button>
  );
}
