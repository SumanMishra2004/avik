    ...
    <>
      <header className="fixed top-0 left-0 right-0 z-50 flex justify-center p-4 sm:p-6 pointer-events-none">
        <motion.nav
          ...
          animate={{ y: 0, opacity: 1, width: scrolled ? "auto" : "100%", maxWidth: scrolled ? "830px" : "1280px" }}
          transition={{ type: "spring", stiffness: 200, damping: 20 }}
          className={`pointer-events-auto flex items-center justify-between gap-6 px-4 py-2.5 rounded-2xl border transition-colors duration-500 ${
            scrolled
        ...
        >
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            >
              <Link href={"/"} className="text-white font-black text-xs">AKD</Link>
            </motion.div>
            {!scrolled && (
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[60] bg-black/40 backdrop-blur-md flex items-center justify-center p-6 md:hidden"
          >
            <motion.div
              initial={{ scale: 0.9, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.9, opacity: 0 }}
              className="w-full max-w-sm bg-[#0A0F1E] border border-[#C8A558]/30 rounded-3xl p-8 shadow-2xl relative"
            >
              ...
              <button onClick={() => setMobileOpen(false)} className="absolute top-6 right-6 text-white/50">
                <X size={24} />
              </button>
            </motion.div>
          </motion.div>
